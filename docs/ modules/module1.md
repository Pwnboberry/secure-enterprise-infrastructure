...
Для обеспечения обратного разрешения IP-адресов была создана зона обратного просмотра (Reverse Lookup Zone) для сети 192.168.10.0/24.
...
Параметр шлюза намеренно не настраивался, поскольку лабораторная сеть является полностью изолированной и не имеет выхода во внешние сети.
